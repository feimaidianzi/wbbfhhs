    ...
    <section className="py-20 bg-secondary">
      <div className="container-custom">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-black text-foreground mb-4">{isZh ? 'UWB 集群组网架构' : 'UWB Swarm Network Architecture'}</h2>
          <p className="text-muted-foreground max-w-2xl mx-auto">{isZh ? '基于 UWB 高精度定位与 WiFi 通讯的分布式集群系统' : 'Distributed swarm system based on UWB high-precision positioning and WiFi communication'}</p>
        </motion.div>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <motion.div initial={{ opacity: 0, y: -20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="flex justify-center mb-8">
            <div className="bg-card border-2 border-accent/40 rounded-xl px-6 py-4 shadow-[0_0_25px_hsl(var(--accent)/0.15)] text-center">
              ...
              <div className="font-bold text-foreground">{isZh ? '地面站 (GCS)' : 'Ground Control Station'}</div>
              <div className="text-xs text-muted-foreground">{isZh ? '编队控制 · 航迹规划 · 数据监控' : 'Formation Control · Path Planning · Data Monitor'}</div>
            </div>
          </motion.div>
          ...
          <div className="flex justify-center mb-4">
            <div className="flex flex-col items-center">
              ...
              <div className="text-[10px] text-accent bg-accent/10 px-2 py-0.5 rounded">{isZh ? 'WiFi 通讯' : 'WiFi Link'}</div>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="flex justify-center mb-8">
            <div className="bg-card border border-accent/30 rounded-xl px-6 py-4 text-center">
              ...
              <div className="font-bold text-sm text-foreground">{isZh ? 'WiFi 路由器 / 通讯节点' : 'WiFi Router / Comm Node'}</div>
              <div className="text-xs text-muted-foreground">{isZh ? '自组网通讯枢纽' : 'Self-organizing communication hub'}</div>
            </div>
          </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            {[1, 2, 3].map((droneId) => (
              ...
              >
                ...
                <div className="font-bold text-foreground mb-1">{isZh ? `无人机 #${droneId}` : `Drone #${droneId}`}</div>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <div className="bg-accent/5 border border-accent/20 rounded-2xl p-6">
              <h3 className="text-lg font-bold text-foreground mb-4 text-center">{isZh ? 'UWB 定位基站层' : 'UWB Positioning Base Station Layer'}</h3>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[1, 2, 3, 4].map((stationId) => (
                  <div key={stationId} className="flex flex-col items-center gap-2 p-3 bg-card rounded-lg border border-accent/10">
                    ...
                    <span className="text-xs font-medium text-foreground">{isZh ? `基站 ${stationId}` : `Station ${stationId}`}</span>
              ...
              <div className="mt-4 text-center text-xs text-muted-foreground">
                {isZh
                  ? '4个UWB基站组成三维定位网络，覆盖范围 ≥500m，支持最多200个标签同时定位'
                  : '4 UWB base stations form a 3D positioning network, coverage ≥500m, supporting up to 200 simultaneous tags'}
              </div>
            </div>
          </motion.div>
        </div>
