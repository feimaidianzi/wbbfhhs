...
export const CertificationsSection = () => {
  const { t } = useLanguage();
  const left = useInViewLite<HTMLDivElement>();
    ...
    <section className="py-16 md:py-20 bg-background border-t border-border/50" id="certifications">
      <div className="container-custom">
        <div className="flex flex-col lg:flex-row items-start lg:items-center gap-12">
          ...
          <div
            ref={left.ref}
            className={`reveal-init reveal-from-left lg:w-1/3 ${left.inView ? 'reveal-in' : ''}`}
          >
          ...
          <div
            ref={right.ref}
            className={`reveal-init reveal-from-right lg:w-2/3 ${right.inView ? 'reveal-in' : ''}`}
          >
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {certifications.map((cert, index) => (
                <div
                  key={index}
                  className="reveal-child group flex items-center gap-4 p-4 bg-secondary/50 rounded-xl hover:bg-secondary transition-colors"
                >
