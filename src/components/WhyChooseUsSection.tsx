...
const stats = [
  { value: "15+", label: "年研发经验", icon: Target },
  { value: "200+", label: "专利技术", icon: Award },
  { value: "500+", label: "企业客户", icon: Users },
  { value: "30+", label: "省市覆盖", icon: Globe },
];
      ...
      <div className="container-custom relative">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-16">
          {stats.map((stat, index) => (
            <div
              key={index}
              className="text-center p-6 bg-primary-foreground/10 rounded-2xl backdrop-blur-sm border border-primary-foreground/20 hover:bg-primary-foreground/15 transition-colors"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {advantages.map((advantage, index) => (
            <div
              key={index}
              className="p-8 bg-primary-foreground/5 rounded-2xl border border-primary-foreground/10 hover:border-accent/50 transition-colors group"
            >
