...
const facilities = [
  { src: officeBuilding, key: "officeBuilding", enLabel: "Company Building" },
  { src: smtWorkshop, key: "smtWorkshop", enLabel: "SMT Workshop" },
  { src: rdOffice, key: "rdOffice", enLabel: "R&D Office" },
  { src: warehouse, key: "warehouse", enLabel: "Warehouse" },
  { src: teamPhoto, key: "teamPhoto", enLabel: "Team Photo" },
  { src: cncWorkshop, key: "cncWorkshop", enLabel: "CNC Workshop" },
  { src: assemblyWorkshop, key: "assemblyWorkshop", enLabel: "Assembly Workshop" },
  { src: aoiInspection, key: "aoiInspection", enLabel: "AOI Optical Inspection" },
];
...
export const FacilityGallery = ({ t }: FacilityGalleryProps) => {
  ...
  // Localized facility label with safe fallback (translation key → English → empty)
  const labelFor = (key: string, enLabel: string) => {
    const tKey = `about.facilities.gallery.${key}`;
    const translated = t(tKey);
    if (translated && translated !== tKey) return translated;
    return enLabel;
  };

  const titleKey = "about.facilities.title";
  const descKey = "about.facilities.desc";
  const title = t(titleKey) !== titleKey ? t(titleKey) : baseLang === "zh" ? "设施与环境" : "Facilities & Environment";
  const desc =
    t(descKey) !== descKey
      ? t(descKey)
      : baseLang === "zh"
      ? "现代化的研发与生产设施"
      : "Modern R&D and production facilities";

  return (
    <section className="py-16 bg-background">
      <div className="container-custom">
        <h2 className="text-2xl md:text-3xl font-bold text-center mb-4">{title}</h2>
        <p className="text-muted-foreground text-center mb-10 max-w-2xl mx-auto">{desc}</p>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {facilities.map((item, index) => {
            const label = labelFor(item.key, item.enLabel);
            return (
              <button
                key={index}
      ...
      {lightboxIndex !== null && (
        ...
        >
          ...
          <p className="absolute bottom-6 left-1/2 -translate-x-1/2 text-white text-lg font-medium bg-black/50 px-4 py-2 rounded-full">
            {labelFor(facilities[lightboxIndex].key, facilities[lightboxIndex].enLabel)}
          </p>
        </div>
      )}
    </section>
